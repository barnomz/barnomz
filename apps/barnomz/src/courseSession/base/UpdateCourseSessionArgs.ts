/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { ArgsType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { CourseSessionWhereUniqueInput } from "./CourseSessionWhereUniqueInput";
import { ValidateNested } from "class-validator";
import { Type } from "class-transformer";
import { CourseSessionUpdateInput } from "./CourseSessionUpdateInput";

@ArgsType()
class UpdateCourseSessionArgs {
  @ApiProperty({
    required: true,
    type: () => CourseSessionWhereUniqueInput,
  })
  @ValidateNested()
  @Type(() => CourseSessionWhereUniqueInput)
  @Field(() => CourseSessionWhereUniqueInput, { nullable: false })
  where!: CourseSessionWhereUniqueInput;

  @ApiProperty({
    required: true,
    type: () => CourseSessionUpdateInput,
  })
  @ValidateNested()
  @Type(() => CourseSessionUpdateInput)
  @Field(() => CourseSessionUpdateInput, { nullable: false })
  data!: CourseSessionUpdateInput;
}

export { UpdateCourseSessionArgs as UpdateCourseSessionArgs };
